...
async function main() {
  ...
  // Auto-write addresses to the frontend config.  Merge instead of overwrite so
  // the MILLOW keys (propertyNft/propertyRegistry/millowEscrow, demoRate,
  // demoRoles) written by scripts/deployMillow.js are preserved.
  const configPath = path.join(__dirname, '..', 'src', 'config.json')
  const existing = JSON.parse(fs.readFileSync(configPath, 'utf8'))
  const chain = (existing['31337'] && typeof existing['31337'] === 'object') ? existing['31337'] : {}
  const config = {
    ...existing,
    "31337": {
      ...chain,
