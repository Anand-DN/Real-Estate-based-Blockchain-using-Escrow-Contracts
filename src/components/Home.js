...
const Home = ({ home, provider, account, escrow, togglePop, setNotification }) => {
    ...
    const fetchDetails = useCallback(async () => {
        ...
        const [lenderFunded, escrowAmt, purchasePrice] = await Promise.all([
            escrow.lenderFunded(propertyId),
            escrow.escrowAmount(propertyId),
        ...
        // retry.  Funding is tracked per property on-chain.
        setHasLended(
            hasLenderApproval &&
            lenderFunded.gte(purchasePrice.sub(escrowAmt)),
        )
    ...
    const lendHandler = async () => {
        ...
        try {
            ...
            // Lender sends funds to the contract, attributed to this sale so a
            // cancel can refund them correctly.
            const lendAmount = (await escrow.purchasePrice(propertyId)).sub(await escrow.escrowAmount(propertyId))
            const fundingTransaction = await escrow.connect(signer).fundSale(propertyId, { value: lendAmount })
    ...
    const sellHandler = async () => {
        ...
        try {
            ...
            const [inspectionOk, buyerApproved, sellerApproved, lenderApproved, earnestPaid, lenderFunded, escrowAmt, price] = await Promise.all([
            ...
            if (!earnestPaid.gte(escrowAmt)) {
                setNotification('Waiting for buyer earnest deposit', 'error')
                return
            }
            if (!lenderFunded.gte(price.sub(escrowAmt))) {
                setNotification('Waiting for lender funds', 'error')
                return
            }
